...
void relax(int** Matr, int shortest[], int pred[], int u, int v)
{
    if (Matr[u][v] != NO_PATH && shortest[u] != NO_PATH) {

        if (shortest[u] + Matr[u][v] < shortest[v]) //выбираем минимальный путь
        {
            shortest[v] = shortest[u] + Matr[u][v]; //переприсваиваем этот путь
            pred[v] = u; //и устанавливаем для него предыдущую вершину
        }//if 2
    }//if 1
